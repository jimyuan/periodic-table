...
export default {
  ...
  computed: {
    ...
    radiation () {
      return Array.isArray(this.element.atomicMass) || [90, 91, 92].indexOf(this.element.atomicNumber) > -1
    }
  },
  methods: {
    goDetail () {
      const eleName = this.element.name
      eleName && this.$router.push({
        path: `/element/${eleName}`
      })
    }
  }
}
